...
static bool waitState(GPIO_PinState pinState, const uint8_t timeout_us);
static bool readResponse(void);
...
static bool readResponse(void) {
  ...
  // 等待 DHT11 拉低（83us）
  if (!waitState(GPIO_PIN_RESET, 100))
    return false;

  // 等待 DHT11 拉高（87us）
  if (!waitState(GPIO_PIN_SET, 100))
    return false;
...
static uint8_t readByte(void) {
  ...
  for (uint8_t i = 0; i < 8; i++) {
    // 等待变为低电平
    if (!waitState(GPIO_PIN_RESET, 60)) {
      // OLED_Show_String(2, 1, "data fail_1");
      return aByte;
    }
    // 等待变为高电平
    if (!waitState(GPIO_PIN_SET, 60)) {
