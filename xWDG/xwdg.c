...
{
  if((HAL_GetTick() - iwdgTimer) > IWDG_INTERVAL_MS){
    iwdgTimer = HAL_GetTick();
...
//   // 当前最短间隔：(4096 * 8) * (127 - 100 + 1) / 36MHz = 25.46ms 3
//   // 当前最长间隔：(4096 * 8) * (127 - 64 + 1) / 36MHz = 58.25ms  7.2
//   // hwwdg.Instance = WWDG;
...
//   // 先写 CFR：配置窗口、预分频
//   WWDG->CFR = WWDG_PRESCALER_8 | 127;  // ÷8, window=127
