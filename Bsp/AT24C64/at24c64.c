...
HAL_StatusTypeDef AT24C64_Write_Bytes(uint16_t page, uint8_t addrInPage, const uint8_t *data, uint8_t size)
{
  ...
  if ((addrInPage + size) > AT24C64_PAGE_SIZE) return HAL_ERROR;
...
{
  if(page >= AT24C64_PAGE_CNT) return HAL_ERROR;
  if(size == 0 || size > AT24C64_MAX_READ_SIZE) return HAL_ERROR;
  if((addrInPage + size) > AT24C64_MAX_ADDRESS_SPACE) return HAL_ERROR;
