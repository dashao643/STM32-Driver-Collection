...
// 写寄存器: 起始时序 -> 从机地址写 -> 寄存器地址 -> 写入的数据 -> 结束时序

// 读寄存器: 起始时序 -> 从机地址写 -> 寄存器地址 -> 结束时序(I2C不需要这一步的结束时序) ->
// 起始时序 -> 从机地址读 -> 读取一个字节 -> 发送一个nack信号(不应答) -> 结束时序

// 一次只能读写1个寄存器

#define SCL_HIGH()  HAL_GPIO_WritePin(SCCB_SCL_GPIO_Port, SCCB_SCL_Pin, GPIO_PIN_SET)
#define SCL_LOW()   HAL_GPIO_WritePin(SCCB_SCL_GPIO_Port, SCCB_SCL_Pin, GPIO_PIN_RESET)
#define SDA_HIGH()  HAL_GPIO_WritePin(SCCB_SDA_GPIO_Port, SCCB_SDA_Pin, GPIO_PIN_SET)
#define SDA_LOW()   HAL_GPIO_WritePin(SCCB_SDA_GPIO_Port, SCCB_SDA_Pin, GPIO_PIN_RESET)
...
static void sendAck(void) UNUSED_FUNC;
